...
/**
 * A factory that creates a {@link SteileRequest} from a {@link Request web request}
 */
public class SteileRequestFactory {
    private final LanguageChecker languageChecker;
    private final static GeometryFactory geometryFactory = new GeometryFactory(new PrecisionModel(), 4326);

    public SteileRequestFactory(Set<String> supportedLanguages) {
        this.languageChecker = new LanguageChecker(supportedLanguages);
    }

    public <R extends SteileRequest> R create(Request webRequest) throws BadRequestException {
